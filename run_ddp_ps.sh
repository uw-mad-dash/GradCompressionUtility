...
OMP_NUM_THREADS=8 python -m torch.distributed.launch --nproc_per_node=4 --nnodes=$num_workers --node_rank=$rank --master_addr=$master_ip --master_port=2345 main_ddp_ps_code.py --batch-size $bsize --dataset-location $dataset_location --log-file $log_file --s3-prefix $s3_prefix --node_rank $rank
